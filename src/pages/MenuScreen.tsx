    ...
    x: '100%'
      ...
      <div className="max-w-md mx-auto min-h-screen pb-20">
        ...
        <header className="sticky top-0 bg-deep-900/80 backdrop-blur-lg border-b border-lavender-faint px-6 py-4 flex items-center gap-4 z-10">
          <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-lavender-faint text-lavender transition-colors">
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-br from-purple-deep to-deep-800 border border-lavender-faint">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-bold text-white">Alex Doe</h2>
              <p className="text-sm text-lavender-muted">
                alex.doe@example.com
              </p>
            </div>
            <button className="px-3 py-1.5 rounded-lg bg-lavender-faint text-xs font-medium text-white hover:bg-lavender-faint/20 transition-colors">
              Edit
            </button>
          </div>
          ...
          <div className="space-y-6">
            {menuGroups.map((group, i) => <div key={i}>
                <h3 className="text-xs font-bold uppercase tracking-wider text-lavender-muted mb-3 px-2">
                  {group.title}
                </h3>
                <div className="space-y-1">
                  {group.items.map((item, j) => <button key={j} className={`w-full flex items-center justify-between p-3 rounded-xl transition-colors ${item.danger ? 'hover:bg-red-500/10 text-red-400' : 'hover:bg-lavender-faint text-lavender hover:text-white'}`}>
                      <div className="flex items-center gap-3">
                        <item.icon className={`w-5 h-5 ${item.danger ? 'text-red-400' : 'text-lavender-muted'}`} />
                        <span className="font-medium text-sm">
                          {item.label}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        {item.value && <span className="text-xs text-lavender-muted">
                            {item.value}
                          </span>}
